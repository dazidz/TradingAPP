...
def save_to_supabase(ticker, signal_type, timestamp):
    try:
        # Duplikat-Prüfung: Gab es heute schon ein Signal für diesen Ticker?
        # Wir prüfen den Tag basierend auf dem Datum des Signals
        ...
        if len(check.data) > 0:
            return # Signal existiert bereits für heute
...
def scan_ticker(ticker):
    ...
    # Scan der letzten 24 Stunden (die letzten 24 Zeilen)
...
if __name__ == "__main__":
    print("🧹 Bereinige alte Signale (>24h)...")
    try:
        ...
        print(f"Hinweis beim Aufräumen: {e}")

    print("🚀 Starte Batch-Scan (letzte 24h)...")
    ticker_liste = get_ticker_list_from_db()
    
    if not ticker_liste:
        print("⚠️ Keine Ticker gefunden.")
    else:
