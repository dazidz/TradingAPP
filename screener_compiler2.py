...
def scan_ticker(ticker):
    ...
    # NEU: Nur das aktuellste Signal suchen (von hinten nach vorne)
    signal_found = False
    for i in reversed(range(len(data))):
        if sE.iloc[i]:
            timestamp = data.index[i]
            save_to_supabase(ticker, "ELITE", timestamp)
            print(f"🔥 {ticker}: Letztes ELITE-Signal auf {timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
            signal_found = True
            break # Stoppt hier nach dem ersten (aktuellsten) Signal
        elif sK.iloc[i]:
            timestamp = data.index[i]
            save_to_supabase(ticker, "KAUFEN", timestamp)
            print(f"💰 {ticker}: Letztes KAUFEN-Signal auf {timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
            signal_found = True
            break # Stoppt hier nach dem ersten (aktuellsten) Signal
