...
def save_to_supabase(ticker, company_name, signal_type, candle_time, sector, gettex_ticker):
    try:
        ...
        print(f"✅ {ticker} ({company_name}) -> Signal: {signal_type}")
    except Exception as e:
...
def scan_ticker(ticker_info):
    ...
    # ADX: Ohne 1.25 Faktor, da dies zu Verzerrungen führte
    adxV = rma(100 * abs(plus_di - minus_di) / (plus_di + minus_di).replace(0, 0.0001), 14)
    ...
    # 4. Sichere Signal-Suche
    signal_found = False # HIER wird die Variable initialisiert!
    ...
    if not signal_found:
        pass # Optional: print(f"ℹ️ {ticker}: Kein Signal.")
...
if __name__ == "__main__":
    ...
    for t_info in ticker_liste:
        try:
            ...
            print(f"❌ Fehler bei {t_info.get('ticker', 'Unbekannt')}: {e}")